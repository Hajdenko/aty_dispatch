    ...
    window.addEventListener("message", function(event){
        ...
        switch(data.action){
            case "dispatch":
                ...
                $.ajax({
                    url:'https://cfx-nui-aty_dispatch/ui/sounds/' + data.sound,
                    type:'HEAD',
                    error: function()
                    {
                        beep.currentTime = "0"
                    ...
                    success: function()
                    {
                        if (audioPlayers["sounds/" + data.sound] == undefined || audioPlayers["sounds/" + data.sound] == null) {
                            let audioPlayer = new Audio("sounds/" + data.sound);
                            audioPlayers["sounds/" + data.sound] = audioPlayer;
                        }
                        audioPlayers["sounds/" + data.sound].load();
                        audioPlayers["sounds/" + data.sound].play();
                    }
                });
                ...
                <div class="dispatch ${random}" id="dis-${dispatchId}">   
                    <div class="header">
                        ...
                        <div class="code${data.panic_effect == true ? " panicEffectCode" : ""}">${data.code}</div>
                    ...
                    <div class="info-wrapper">
                        ...
                        ${data.distance ? 
                            `<div class="info distance">
                                ...
                                <div class="text">${Math.round(data.distance)}m Daleko od Tebe</div>
                            </div>` 
                            : ""
                        }
                        ${data.gender ? 
                    ...
                    <div class="time-sent">
                        ...
                        <div class="time" data-time="${Date.now()}">Zrovna teď</div>
                    </div>
                </div>
                `
            ...
            case "showDispatch":
                ...
                $(".showDispatch>div").each((_, el) => {
                    ...
                    $el.find(".time").text(getRemaining(now - other))
                })
